    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        @foreach($menu_coffe as $menu)
            <div class="bg-white border rounded-2xl shadow-sm hover:shadow-md transition p-4">
                ...
                @if(auth()->check())
                    <!-- Sudah login → arahkan ke halaman order -->
                    <a href="{{ route('order', $menu->id) }}"
                       class="bg-[#6f4e37] w-full text-white py-2 rounded-lg hover:bg-[#5a3e2b] transition inline-block text-center">
                       Order Now
                    </a>
                @else
