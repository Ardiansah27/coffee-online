...
<div class="md:hidden flex flex-col items-center justify-center min-h-screen bg-cover bg-center"
  ...
  <div class="w-full px-6">
    ...
    <!-- Form Mobile -->
    <form class="space-y-4" autocomplete="off" method="POST" action="{{ route('login.submit') }}">
      @csrf
      ...
      <div class="relative">
        ...
        <p class="text-right mt-2">
          <a href="{{ route('forgot.password') }}" class="text-lg text-[#5A3A1A] hover:underline">Lupa Password?</a>
        </p>
      </div>
      @error('password')
          <p class="text-red-500 text-sm">{{ $message }}</p>
...
<div class="hidden md:flex items-center justify-center min-h-screen bg-cover bg-center"
  ...
  <div class="bg-[#A15F3B] bg-opacity-90 rounded-3xl shadow-2xl p-10 w-1/3">
   ...
   <h1 class="text-4xl font-bold text-white mt-4 w-full text-center mb-3">Login</h1>
    ...
    <!-- Form Desktop -->
    <form class="space-y-6" autocomplete="off" method="POST" action="{{ route('login.submit') }}">
      @csrf

      <!-- Email -->
      <div class="relative">
        <img src="{{ asset('uploads/email1.png') }}" alt="Email Icon" class="absolute left-4 top-1/2 transform -translate-y-1/2 w-6 h-6">
        <input type="email" id="email" name="email" placeholder="Massukan Email Anda" value="{{ old('email') }}" autocomplete="off"
      ...
      <div class="relative">
        ...
        <p class="text-right mt-2">
          <a href="{{ route('forgot.password') }}" class="text-lg text-white hover:underline">Lupa Password?</a>
        </p>
      </div>
      @error('password')
          <p class="text-red-500 text-sm">{{ $message }}</p>
      ...
      <!-- Google Login -->
      <button class="w-full flex items-center justify-center gap-3 bg-white text-gray-800 font-semibold py-3 rounded-full ring-2 ring-[#5A3A1A] shadow-md transition duration-300 hover:bg-[#3f2612] hover:text-white mt-4"
              onclick="window.location='{{ route('login.google') }}';">
        <img src="{{ asset('uploads/google2.png') }}" alt="Google Logo" class="w-5 h-5">
        <span>Login dengan Google</span>
      </button>
...
<!-- UBAH / TAMBAHKAN: bagian ini supaya notifikasi muncul -->
@if(session('success'))
<script>
  Swal.fire({
    title: 'Berhasil!',
